...
class RunReporter:
    ...
    def on_result(self, result: PromptRunResult) -> None:
        if result.status == "done":
            ...
            print(
                f"\n[ERROR] {result.video_name} + '{result.prompt}'\n"
                f"{result.error}\n"
            )
...
def run_single_prompt_job(
    ...
    if config.skip_existing and job.output_path.exists():
        return PromptRunResult.skipped(job)
...
def run_video_jobs(
    ...
    video_frames_for_vis = read_video_frames_rgb(video_path)
